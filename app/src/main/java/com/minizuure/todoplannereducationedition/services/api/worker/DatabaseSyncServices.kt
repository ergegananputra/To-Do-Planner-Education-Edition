...
class DatabaseSyncServices : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when(intent?.action) {
            ...
            SyncType.UPLOAD.toString() -> uploadDatabase()
    ...
    private fun uploadDatabase() {
        ...
        try {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                val mainTask = this.launch(Dispatchers.IO) {
                            ...
                            for (note in todo) {
                                ...
                                community.todo_notes_references!!.collection(fireNote.table)
                                    ...
                                    .addOnSuccessListener {
                                        ...
                                        Log.e("DatabaseSyncServices", "Total databaseJobs: ${databaseJobs.size}")
                ...
                mainTask.join()
                databaseJobs.forEachIndexed { index, job ->
                    job.join()
                    Log.v("DatabaseSyncServices", "Job $index out of ${databaseJobs.size} done, status: ${job.start()}")
                }
                stopSelf()
